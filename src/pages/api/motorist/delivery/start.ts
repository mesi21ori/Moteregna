  ...
  try {
    ...
    if (req.body.isAvailable !== undefined) {
      ...
      if (motorist.userId !== decoded.userId) {
        return res
          .status(403)
          .json({ message: "Forbidden: You do not have permission to create a delivery for this motorist" })
      }

      // Check if motorist is online first
      if (!motorist.isOnline) {
        return res.status(400).json({ message: "Motorist is offline. Cannot start delivery." })
      }
      ...
      // First try to find an active price
      ...
      // If no active price exists, fall back to the most recent price
      if (!price) {
