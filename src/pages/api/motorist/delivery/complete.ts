...
const updateDeliverySchema = z.object({
  ...
  status: z.enum(["PENDING", "ASSIGNED", "PICKED_UP", "IN_TRANSIT", "DELIVERED", "CANCELLED"]),
  endTime: z.string().min(1, "End time is required"),
  destination: z.string().min(1, "destination location is required"), // Optional destination
  destinationLat: z.number().min(1, "destination latitude location is required"),  // Optional destination latitude
  destinationLong: z.number().min(1, "destination longitude is required"),  // Optional destination longitude
});
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Create end location only if the delivery is being marked as DELIVERED
    ...
    const updatedDelivery = await prisma.delivery.update({
      ...
      data: {
        ...
        status: validatedData.status,
        endTime: new Date(validatedData.endTime),
        endLocationId: endLocationId, // Update endLocationId if created
      },
    });
