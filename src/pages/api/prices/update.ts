...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // If this price is being set as active, deactivate all other prices
    if (validatedData.isActive) {
      await prisma.price.updateMany({
        where: {
          userId: decoded.userId,
          isActive: true,
          id: { not: existingPrice.id },
        },
        data: {
          isActive: false,
        },
    ...
    const updatedPrice = await prisma.price.update({
      ...
      data: {
        ...
        perminute: validatedData.perMinutePrice ?? existingPrice.perminute,
        isActive: validatedData.isActive ?? existingPrice.isActive,
        isActiveDate: validatedData.isActive && !existingPrice.isActive ? new Date() : existingPrice.isActiveDate,
        userId: decoded.userId,
      },
      include: {
        user: {
    ...
    return res.status(200).json({
      ...
      data: {
        id: updatedPrice.id,
        basePrice: updatedPrice.Price,
        perKmPrice: updatedPrice.perkilometer,
        perMinutePrice: updatedPrice.perminute,
        isActive: updatedPrice.isActive,
        isActiveDate: updatedPrice.isActiveDate,
